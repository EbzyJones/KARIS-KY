...
/// | 7 | Added `VersionHistory`, `SanctionsProvider`, `AttestationTimestamp`, `AttestationTag`; sanctions screening; attestation log query | Additive keys — no `migrate` call required |
///
...
pub enum EscrowError {
    ...
    /// Sanctions screening blocked an address that appears on the configured sanctions list.
    /// The provider contract returned `true` for the checked address.
    AddressOnSanctionsList = 165,
}
...
pub enum DataKey {
    ...
    /// Append-only audit trail of (version, ledger_timestamp) tuples tracking deployment lineage.
    /// Written on init, migrate, and upgrade. Absent ⇒ empty vec.
    /// See [`LiquifactEscrow::get_version_history`].
    VersionHistory,
    /// Optional sanctions list provider contract address. Set at init, immutable thereafter.
    /// Absent ⇒ no sanctions screening on fund/settle.
    SanctionsProvider,
    /// Per-index ledger timestamp for [`DataKey::AttestationAppendLog`] entries.
    /// Absent ⇒ 0 (no timestamp recorded). Stored once per append at the same index.
    AttestationTimestamp(u32),
    /// Optional per-index tag for [`DataKey::AttestationAppendLog`] entries.
    /// Absent ⇒ no tag. Stored once per append when tag is provided.
    AttestationTag(u32),
}
...
impl LiquifactEscrow {
    ...
    pub fn init(
        ...
        sanctions_provider: Option<Address>,
    ) -> InvoiceEscrow {
        admin.require_auth();
