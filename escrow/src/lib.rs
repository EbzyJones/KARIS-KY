...
pub enum EscrowError {
    ...
    /// The escrow is paused by admin; all state-mutating entrypoints are blocked.
    EscrowIsPaused = 165,
}
...
pub enum DataKey {
    ...
    /// Bucketed aggregate of all investor contributions that hash into this bucket.
    /// 256 buckets indexed 0..255 (see [`INVESTOR_BUCKET_COUNT`]).
    /// Each entry stores the sum of contributions whose `address_hash % 256 == index`.
    /// **Instance** storage. Absent ⇒ `0`.
    /// Written atomically with [`DataKey::InvestorContribution`] on every `fund` / `fund_with_commitment`.
    /// Enables O(1) estimates for dashboards without iterating per‑investor persistent keys.
    InvestorContributionBucket(u32),
    /// When true, all state-mutating entrypoints are blocked.
    /// Absent ⇒ `false` (not paused). Toggled by admin via [`LiquifactEscrow::set_escrow_paused`].
    EscrowPaused,
}
