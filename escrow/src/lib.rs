...
pub enum DataKey {
    ...
    /// Append-only audit trail of an investor's contribution snapshots, recorded at
    /// **checkpoints**: each [`LiquifactEscrow::fund`] / [`LiquifactEscrow::fund_with_commitment`]
    /// deposit (tagged `"deposit"`, or `"funding_close"` when that deposit closes funding), and
    /// each [`LiquifactEscrow::claim_investor_payout`] (tagged `"settlement"`).
    /// **Persistent** storage. Absent ⇒ empty history. Bounded by [`MAX_INVESTOR_HISTORY_ENTRIES`].
    InvestorFundingHistory(Address),
    /// Optional KYC/identity-verification registry contract; set once at
    /// [`LiquifactEscrow::init`]. When set, [`LiquifactEscrow::fund`] and
    /// [`LiquifactEscrow::fund_with_commitment`] reject deposits from investors the registry
    /// reports as unverified. Absent ⇒ KYC gating disabled (any address may fund).
    KycProviderContract,
    /// Maps admin addresses to their [`AdminRole`] tier. Absent ⇒ no tiered roles configured;
    /// [`InvoiceEscrow::admin`] is treated as implicit [`AdminRole::Full`] (backward compatible).
    /// See [`LiquifactEscrow::set_admin_roles`] and [`LiquifactEscrow::get_admin_role`].
    AdminRoles,
    /// Configured [`MultisigPolicy`] gating critical operations (e.g. legal hold changes, large
    /// claim releases). Absent ⇒ no policy configured; gated entrypoints fall back to requiring
    /// the single [`InvoiceEscrow::admin`] signature. See [`LiquifactEscrow::init_multisig_policy`].
    MultisigPolicy,
}
...
impl LiquifactEscrow {
    ...
    pub fn claim_investor_payout(env: Env, investor: Address) {
        ...
        // Record the settlement checkpoint (see #148) before any reinvestment side effects.
