...
/// Build metadata embedded in the WASM artifact by `escrow/build.rs`.
///
/// Returned by [`LiquifactEscrow::get_build_metadata`] so off-chain tooling and
/// CI can verify a deployed contract was built from the expected source commit,
/// with the expected [`SCHEMA_VERSION`] and [`CONTRACT_INTERFACE_VERSION`].
///
/// Additive entrypoint: does **not** require a `SCHEMA_VERSION` bump because it
/// only reads compile-time constants and exposes no new on-disk storage.
/// See ADR-007 `Rule 1` (new optional keys are backward-compatible) — the same
/// principle applies to new entrypoints that do not modify storage shape.
#[contracttype]
#[derive(Clone, Debug, PartialEq)]
pub struct BuildMetadata {
    /// [`SCHEMA_VERSION`] baked into the WASM (matches the source constant).
    pub schema_version: u32,
    /// [`CONTRACT_INTERFACE_VERSION`] baked into the WASM (matches the source constant).
    pub interface_version: u32,
    /// Short git commit hash (`git rev-parse --short HEAD`); `"unknown"` when unavailable.
    pub git_commit: String,
    /// Full git commit hash (`git rev-parse HEAD`); `"unknown"` when unavailable.
    pub git_full_commit: String,
    /// ISO-8601 UTC timestamp at compile time; `"unknown"` when unavailable.
    pub build_timestamp: String,
    /// Cargo package version (`CARGO_PKG_VERSION`).
    pub pkg_version: String,
    /// Rust compiler version (`rustc --version`); `"unknown"` when unavailable.
